                            ...
                            GetBitContext *gb_host, int crc, int cnt, int id_aac)
{
    ...
    /* The original FFmpeg reader has a padded input contract. LogitOS accepts
     * untrusted network frames with an exact bound instead, so propagate an
     * attempted over-read out of the private copy rather than letting a
     * truncated payload degrade into plausible-sounding pure upsampling. */
    if (gb->error) {
        gb_host->error = 1;
        sbr_turnoff(sbr);
    }
