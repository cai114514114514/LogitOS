...
#endif
#ifdef TCC_LOGIT
    /* LogitOS has no dynamic loader: c/kernel/exec/elf.c refuses PT_INTERP
     * and PT_DYNAMIC by name, so the only executable that can run is a static
     * one. Default to it. The rejected alternative was to keep the stock
     * default (dynamic, with CONFIG_TCC_ELFINTERP naming an ld.so) and rely
     * on every user passing -static: the failure is then a link that
     * succeeds, writes an ELF with an .interp section, and is refused at
     * exec time with a message about a loader the user never asked for. */
    s->static_link = 1;
#endif
    tccelf_new(s);
    tccpp_new(s);
...
# if defined TCC_LOGIT
    /* The OS macro describes the TARGET. Stock tcc keys `__linux__` on the
     * compiler that built tcc itself, which is right for a native compiler
     * and wrong for this one: built on a Linux host it would stamp every
     * LogitOS program `__linux__`, and built on the device it would stamp
     * nothing at all, so a port's `#ifdef __linux__` would flip depending on
     * where tcc was compiled. `__logit__` follows the `__FreeBSD__` slot
     * below; nothing in c/apps/libc keys on it yet (grepped 2026-08-21), so
     * this is the name being claimed, not a name being honoured. */
    tcc_define_symbol(s, "__logit__", NULL);
...
#ifdef TCC_LOGIT
            /* Not "unsupported": a shared object is a file the kernel's
             * loader refuses (ET_DYN needs relocation, and elf.c applies
             * none), so producing one would be a link that succeeds and an
             * exec that fails elsewhere. With static_link forced on above,
             * the DLL path would also emit a .so with no .dynamic at all --
             * a file no loader anywhere could use. Refuse here, by name. */
            tcc_error("-shared: LogitOS has no dynamic loader; "
                      "link a static executable instead");
#endif
            x = TCC_OUTPUT_DLL;
