...
#include "tcc.h"
#ifdef TCC_LOGIT
# include <sys/stat.h>   /* chmod(): see tcc_write_elf_file */
#endif
    ...
    for(i = 1; i < s1->nb_sections; i++) {
        ...
        for_each_elem(s, 0, rel, ElfW_Rel) {
		 ...
		 ELFW(ST_BIND)(sym->st_info) == STB_LOCAL)) {
            ...
            /* A STATIC EXECUTABLE HAS NO PLT. Every address is final at link
               time, so a call is a direct rel32 to its target. Upstream
               0.9.27 routed PLT32 to a PLT stub here anyway, and the static
               path then never completed it: put_got_entry() does not record
               got_offset for a PLT slot, so fill_got() wrote nothing into
               the slot, and relocate_plt() -- which turns the stub's
               `jmp *got_offset(%rip)` into a real displacement -- is only
               called under `if (dynamic)`. The stub therefore read its
               target out of the .plt bytes themselves: measured on the
               first cross-object call of a tcc-linked hello on LogitOS,
               rip=0x2825ffffff, which is the little-endian bytes of the
               next PLT entry. The rejected alternative was to complete the
               static PLT (fill the slot, relocate the stub); that is more
               code to produce an indirection no static program needs. */
            if (s1->static_link && s1->output_type == TCC_OUTPUT_EXE &&
                           ...
                           struct dyn_inf *dyninf, int *sec_order)
{
    ...
    ElfW(Phdr) *ph;
    Section *s;
    /* the TLS template: address, file offset, initialised size, total size,
       alignment -- see the .tdata/.tbss notes in the loop and the PT_TLS
       fill after it */
    int have_tls = 0, tls_off = 0;
    unsigned long tls_align = 0;
    addr_t tls_addr = 0, tls_filesz = 0, tls_memsz = 0;
    ...
    if (phnum > 0) {
        ...
        for(j = 0; j < 2; j++) {
            ...
            /* Six passes, not upstream's five: pass 3 is the TLS template
               (.tdata), placed FIRST among a segment's PROGBITS so that
               .tbss, handled in the NOBITS pass below, can continue the
               template directly after it. See the .tbss note. */
            for(k = 0; k < 6; k++) {
                for(i = 1; i < s1->nb_sections; i++) {
                    s = s1->sections[i];
                    ...
                    if (s == interp) {
                        ...
                        if (k != 2)
                            continue;
                    } else if ((s->sh_flags & SHF_TLS) && s->sh_type != SHT_NOBITS) {
                        if (k != 3)
                            continue;
                    ...
                    if ((s->sh_flags & SHF_TLS) && s->sh_type == SHT_NOBITS) {
                        /* .tbss IS A TEMPLATE, NOT MEMORY. Every thread gets
                           its own zeroed copy of these bytes, laid out by
                           whoever creates the thread (the kernel's loader for
                           the main thread, from PT_TLS), so the section takes
                           NO address space in the segment: its address is
                           where the template continues after .tdata, and it
                           overlaps whatever the segment places next -- GNU
                           ld's layout exactly. The rejected alternative,
                           letting it advance `addr` like .bss, wastes its
                           size in every process and, worse, puts .data
                           between .tdata and .tbss so the template is no
                           longer contiguous and PT_TLS cannot describe it. */
                        if (!have_tls) {
                            have_tls = 1;
                            tls_addr = (addr + s->sh_addralign - 1) & ~(addr_t)(s->sh_addralign - 1);
                            tls_off = file_offset;
                            tls_align = 1;
                        }
                        if (s->sh_addralign > tls_align)
                            tls_align = s->sh_addralign;
                        tmp = tls_addr + tls_memsz;
                        tmp = (tmp + s->sh_addralign - 1) & ~(long long)(s->sh_addralign - 1);
                        s->sh_addr = tmp;
                        s->sh_offset = file_offset;     /* nothing is written */
                        tls_memsz = tmp + s->sh_size - tls_addr;
                        continue;
                    }
                    ...
                    /* .tdata: the initialised part of the TLS template.
                       First of its segment's PROGBITS by the pass order
                       above, so the template starts here. */
                    if (s->sh_flags & SHF_TLS) {
                        ...
                        if (s->sh_addralign > tls_align)
                            tls_align = s->sh_addralign;
                        tls_filesz = addr + s->sh_size - tls_addr;
        ...
        /* THE TLS SEGMENT, in the slot after the two PT_LOADs (elf_output_file
           counted it into phnum). The x86-64 psABI's variant II puts the
           thread pointer at the END of the block and the template grows
           down from it, so a thread-local symbol's offset from %fs is
           S - (template address + size rounded up to the alignment); that
           rounded end is what relocate() needs, and it is recorded here
           because only the layout knows it. p_align is never below 8: the
           consumer (c/kernel/exec/elf.c) rounds the block to max(p_align, 8)
           and the two computations have to agree, so the file says 8 rather
           than relying on the reader to bump it. */
        if (have_tls) {
            if (tls_align < 8)
                tls_align = 8;
            ph->p_type = PT_TLS;
            ...
            ph->p_vaddr = tls_addr;
            ph->p_paddr = tls_addr;
            ph->p_filesz = tls_filesz;
            ph->p_memsz = tls_memsz;
            ph->p_flags = PF_R;
            ph->p_align = tls_align;
            s1->tls_end = tls_addr + ((tls_memsz + tls_align - 1) & ~(addr_t)(tls_align - 1));
            ph++;
        }
    }
...
#ifdef TCC_LOGIT
    /* The `mode` handed to open() above is the execute bit, and on LogitOS it
     * goes nowhere: SYS_OPEN is (path, flags) -- mini-libc's open() accepts
     * the third argument and discards it (c/apps/libc/src/io.c) -- so a file
     * tcc has just linked is created with the stored-nothing default and
     * execve() refuses it with "permission denied (not executable)". SYS_CHMOD
     * is the call that reaches the inode, so it is made here, after the bytes
     * are on disk. The rejected alternative was to leave it to the user: a
     * compiler whose output cannot be run without a second command is a
     * compiler that looks broken to exactly the person trying it first. */
    if (file_type != TCC_OUTPUT_OBJ)
        chmod(filename, mode);
#endif
...
/* Does the link carry a thread-local template? The same predicate
   layout_sections applies section by section (an ALLOC section carrying
   SHF_TLS, whatever its size), so the PT_TLS slot counted here is exactly the
   one that will be filled there. */
static int tls_sections(TCCState *s1)
{
    int i;
    for (i = 1; i < s1->nb_sections; i++)
        if ((s1->sections[i]->sh_flags & (SHF_ALLOC | SHF_TLS)) == (SHF_ALLOC | SHF_TLS))
            return 1;
    return 0;
}
...
static int elf_output_file(TCCState *s1, const char *filename)
{
    ...
    if (file_type == TCC_OUTPUT_OBJ)
        ...
        phnum = 5;
    /* One more for PT_TLS when the inputs carry a thread-local template.
       Upstream 0.9.27 had no TLS at all in the link editor: a `__thread`
       object in any input -- mini-libc's pthread.o has one, and malloc's
       lock pulls pthread.o into every program that prints -- died with
       "Unknown relocation type: 23". Local-exec TLS in a static executable is
       a layout (here), a segment (layout_sections) and one relocation
       (R_X86_64_TPOFF32 in x86_64-link.c); initial-exec via the GOT is not
       implemented and is refused there rather than mis-linked. */
    s1->tls_end = 0;
    if (file_type != TCC_OUTPUT_OBJ && tls_sections(s1))
        phnum++;
    ...
    if (file_type != TCC_OUTPUT_OBJ) {
        ...
        /* Perform relocation to GOT or PLT entries -- BEFORE the section
           headers are tidied, not after as upstream had it. fill_got() finds
           the GOT slots to fill by walking the .rela.* sections, and for an
           executable those carry no name (alloc_sec_names leaves non-ALLOC
           relocation sections unnamed), so tidy_section_headers() moves them
           past nb_sections and a walk that runs afterwards sees none of them:
           every GOT slot stays zero. Observed, not inferred -- hello's 85-slot
           .got was 680 bytes of zeroes. The order here is the one in which
