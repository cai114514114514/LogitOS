...
/* LogitOS (TCC_LOGIT) is a BATCH compiler, never a native one, even though the
 * binary runs on the same x86-64 it generates code for. TCC_IS_NATIVE would
 * pull in tccrun.c (the -run JIT), which needs <sys/ucontext.h>, sigaction
 * with SA_SIGINFO and an executable heap -- none of which mini-libc or the
 * kernel's W^X loader provide -- and it would switch on CONFIG_TCC_BACKTRACE
 * and CONFIG_TCC_BCHECK, whose runtime halves live in lib/bcheck.c and are
 * not shipped. The rejected alternative was to compile tccrun.c with stubs:
 * then `tcc -run` would link, start, and fault, where today it says "-run is
 * not available in a cross compiler" and exits. A refusal that names itself
 * beats a crash that does not. */
/* only native compiler supports -run */
#if defined _WIN32 == defined TCC_TARGET_PE && !defined TCC_LOGIT
...
/* LogitOS (TCC_LOGIT): where the on-disk sysroot IS, not where configure
 * guessed. These are the exact paths tools/mksysroot.py (the sysroot line)
 * populates -- /usr/include for the headers, /usr/lib for libc.a and the
 * crt objects, /usr/lib/tcc for libtcc1.a -- so a default `tcc hello.c`
 * finds everything with no -I/-L/-B. The stock defaults below would search
 * /usr/local/lib/tcc and /usr/local/include, which do not exist on this disk,
 * and a tcc that needs three flags to compile hello world is a tcc whose
 * first user concludes it is broken. Defined HERE, under #ifndef guards the
 * stock file already has, rather than in a generated config.h: the build is
 * then reproducible from the tree and the choice is argued beside the value.
 * CONFIG_TCC_ELFINTERP is deliberately NOT defined: output is static
 * (libtcc.c forces static_link), the kernel has no dynamic loader, and a
 * path to one would be a lie the next reader believes. */
#ifdef TCC_LOGIT
# ifndef CONFIG_TCCDIR
#  define CONFIG_TCCDIR "/usr/lib/tcc"
# endif
# ifndef CONFIG_TCC_SYSINCLUDEPATHS
#  define CONFIG_TCC_SYSINCLUDEPATHS "/usr/include:{B}/include"
# endif
...
struct TCCState {
    ...
    /* End of the PT_TLS template (its address plus its size rounded up to
       its alignment): where the thread pointer sits relative to the template
       in the x86-64 variant-II layout, so that TPOFF(S) = S - tls_end. 0 when
       the link has no TLS. Set by layout_sections, read by relocate(). */
    addr_t tls_end;
