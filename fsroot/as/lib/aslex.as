# aether: 3.0
# Native batch lexer. Indentation, continuation and literal scanning follow
# frontend/lexer.c so the compiler and tools see the same token boundaries. String
# interiors retain escapes: decoding belongs to the expression parser.
#
# Token = [type, text, line, start_byte, end_byte]. Keep this public sequence
# shape for compiler consumers; its mixed fields explicitly use Any. Scanner
# state and arithmetic have concrete native types. Failures raise ValueError
# and retain error_span on the Lexer for callers that display diagnostics.
...
class Lexer:
    ...
    def at(self, i: i64) -> i64:
        return ord(self.src[i]) if i < self.n else 0

    def emit(self, t: i64, text: str) -> None:
        # Trailing fields are source byte offsets. The first three fields stay
        # compatible with the self-host lexer oracle and bytecode compiler.
        self.toks.append([Any(t), Any(text), Any(self.line), Any(self.start), Any(self.i)])

    def err(self, msg: str) -> None:
        self.error_span = [self.start, self.start + 1]
        raise ValueError(msg + " (line " + str(self.line) + ")")
    ...
    # Parser.err_at in asc.as -- and of error_at(Token, ...) in legacy/compiler.c,
    ...
    def err_at(self, line: i64, msg: str) -> None:
        self.error_span = [self.start, self.start + 1]
        raise ValueError(msg + " (line " + str(line) + ")")
    ...
    def fstring_body(self, q: i64) -> str:
        start = self.i
        ...
        while true:
            ...
            if is_digit(c) or (c == 46 and is_digit(self.at(self.i + 1))):
                ...
                if c == 40:
                    ...
                    self.err("unexpected character '" + chr(c) + "'")
    ...
    def run(self) -> List[List[Any]]:
        ...
        nt = len(self.toks)
        self.start = self.i
        if nt > 0 and cast[i64](self.toks[nt - 1][0]) != T_NEWLINE:
            self.emit(T_NEWLINE, "")
