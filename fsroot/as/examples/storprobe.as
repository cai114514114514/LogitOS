...
def nonzero_hole(from_i, to_i):
    # count non-zero bytes in [from_i, to_i) of RD_BUF
    bad = 0
    for i in range(from_i, to_i):
        if peek8(addr(RD_BUF) + i) != 0:
            bad = bad + 1
    return bad
...
file_write("/sp/rep-target", "OLD")
file_write("/sp/rep-src", "NEW")
...
p = run(["/bin/as", "/usr/as/examples/storchild.as"])
p.start()
code = p.wait()
