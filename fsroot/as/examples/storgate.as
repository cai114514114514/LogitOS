...
# A3 keeps that requirement and adds a reason: the read helpers below are module
# functions, and a buffer built inside main() would not be in scope for them.
BUF = buffer(8192)
...
# These two return bool rather than A2's 0/1 int. A3 has a real bool and will
# not silently read an int as a truth value, which is what let the A2 version
# write `check(w == 300 and bytes_are(...))` -- one bool and one int joined by
# `and`. Returning bool makes that line typecheck instead of coercing.
def bytes_are(ch: str, from_i: i64, to_i: i64) -> bool:
    unsafe:
        ok = True
        want = ord(ch)
        for i in range(from_i, to_i):
            if i64(peek8(addr(BUF) + u64(i))) != want:
                ok = False
        return ok

def zeros(from_i: i64, to_i: i64) -> bool:
    unsafe:
        ok = True
        for i in range(from_i, to_i):
            if i64(peek8(addr(BUF) + u64(i))) != 0:
                ok = False
        return ok
