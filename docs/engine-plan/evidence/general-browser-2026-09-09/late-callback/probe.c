 ...
 * host park seam: stop at wait_idle(0), instead of spinning until an unrelated
 * later poll accidentally consumes work that the guest cannot wake for. */
#define main loader_existing_main
#include "loader_test.c"
#undef main
#include "js_dom.h"
void app_main(void);
static int mode, stage, steps, parked, hook_seen;
...
void loader_poll_hook(void) {
    host_clock+=5;
    if(++steps>500) {CHECK(0,"bounded loop reached park boundary");app_exit(0);}
...
void late_callback_park(int ms) {
    if(!stage)return;
    /* A timeout would hide the same bug more slowly; fixtures have no timer,
     * fetch or animation, and must be complete at the first indefinite park. */
    if(ms)return;
    parked=1;
    ...
    if(mode==1)CHECK(js_int("window.inserted===42"),"late inserted script executed before indefinite park");
    else CHECK(dest==1,"late callback navigation consumed before indefinite park");
...
int main(int argc,char **argv) {
    ...
    CHECK(parked,"observer intercepted actual indefinite park");
    js_page_close();
