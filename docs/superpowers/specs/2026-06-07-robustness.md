---
title: AquaScript robustness batch (stack guard / OOM / wide const / import cleanup)
status: spec
date: 2026-06-07
---

# AquaScript Robustness Batch — Spec

External code-review hardening of the AquaScript bytecode VM (`src/apps/aqs/`). Four LOCKED items, no feature creep. Every change is grounded in verified line anchors in the real tree.
...
| B | Wide constant index (1-byte → 2-byte) | `compiler.c`, `vm.c`, `aqs.h` (version bump) |
| C | OOM checks + propagation | `object.c`, `vm.c`, `compiler.c`, `lexer.c`, `aqs.h` |
| D | Import-failure cleanup | `vm.c` |
| T | Tests | `tools/t/aqs_test.c` |
...
**Behavior.** Stack overflow becomes a CATCHABLE runtime error (folds through `ensure_exc` into `g_exc`); a `try/except` around deep recursion catches it; uncaught it leaves "stack overflow" in `aqs_err` and `run_until` returns 1.

---

## Item B — Wide constant index (CHOSEN: uniform 16-bit)

**Bug.** `makeConst` errors at `k > 255` (`compiler.c:58`) because const-pool operands are one `uint8_t`. A large top-level module pools all its constants in one `ObjFn`, exceeding 255 distinct entries (every unique identifier/method/global/string + each nested `ObjFn` burns a slot; dedup is by `aqs_value_eq`, `object.c:279).
...
**.la round-trip + version (`aqs.h`, `aqs_bc.c`).** `aqs_bc.c` serializes `fn->code` verbatim (`wr_bytes`/`rd_bytes`, lines 63/176) and `fn->consts` as tagged values — the operand bytes live inside `fn->code`, so a 1→2 byte operand round-trips for free. BUT the on-disk `code` layout changes, so bump `AQS_BC_VERSION` `1u → 2u` (`aqs.h:162`). `aqs_load` already rejects mismatched versions (`aqs_bc.c:245`); the `bc_reject` negative test (`aqs_test.c:157-190`) tampers `blob[4]` relative to the current version, so it stays green. The `bc_tests()` roundtrip suite (dump→free→load→run, compares output) is the correctness gate; a `>255`-const roundtrip case is added.
...
**Fix.**
1. `aqs_malloc(size_t)` / `aqs_realloc(void*,size_t)` in `object.c`: on NULL set `aqs_err = "out of memory"` and `g_oom = 1`; return the (NULL) pointer. `static int g_oom;` in `object.c`; declare `extern int g_oom;` in `aqs.h`; reset in `reset_stack()` (`vm.c:61`) AND at the top of `aqs_compile_module` (`compiler.c:775`).
...
4. Propagation by layer:
   - **VM-runtime object.c allocs:** `alloc_obj` returns NULL on fail (g_oom set); DISPATCH catches at the next boundary. Guard every "store after grow" with `if (g_oom) return;` (e.g. `aqs_list_push`, `aqs_module_slot`, `dict_grow`) so no NULL is dereferenced before the unwind. No public signature changes required.
   - **`str_concat` (vm.c:103-109):** `if (!buf) goto err;` (called from `op_ADD`, which has `err:` in scope; g_oom already set).
   - **`module_source` (vm.c:129-158):** wrappers set g_oom and return NULL; `aqs_import` (338) already checks `!src`.
   - **Compile-time:** `string()` (compiler.c:179) → `if (!buf) { error("out of memory"); return; }` (sets `had_error`). `aqs_chunk_write`/`aqs_chunk_const` (object.c) set `g_oom` + SKIP the write on realloc-fail; `aqs_compile_module` checks `g_oom` after the parse loop and returns NULL if set. Incomplete, never-executed bytecode is reclaimed by `aqs_free_objects`.
   - **Lexer `push` (lexer.c:13-21):** `aqs_lex` polls `g_oom` in its scan loop and treats it like its existing `err` flag → returns NULL → `aqs_compile_module:779` `if (!toks) return NULL`.
   - **GC gray stack (object.c:293-297) — hard case:** on `aqs_realloc` NULL during mark: do NOT advance `gray_cap`, set `o->marked = 0` (un-mark → sweep treats it conservatively LIVE; a leaked object is recoverable next GC, a freed-live object is UAF), set `g_oom = 1`, return without enqueuing. Collection finishes with a possibly-incomplete worklist but NO freed-live objects and NO NULL deref; control returns to `alloc_obj` → opcode → DISPATCH → unwind.
5. **Harden the error-reporting allocs:** `runtime_error` (vm.c:90-96) and `ensure_exc` (vm.c:85-88) call `aqs_str_copy`, which can OOM. If it returns NULL, fall back to `throw_value(NIL_VAL)` rather than storing `OBJ_VAL(NULL)` (which a catcher could deref). `gc_mark_obj` is already NULL-safe (vm.c:291) and the uncaught `IS_STR(g_exc)` path (vm.c:807) handles a non-string `g_exc`.
...
**Bug.** `aqs_import` (`vm.c:328-345`) registers `modules[nmodules++] = m` at line 335 BEFORE compile/run (intentional, circular-safe), but the three failure paths (`!src` 338, `!script` 341, `run_module` fail 342) `return NULL` leaving the broken partial module in `modules[]` — a later `import` returns the dud.
...
## Test plan (`tools/t/aqs_test.c`; current count 135)
...
- **B (.la):** a `roundtrip("bc_wide_const", <same 300-const source>)` inside `bc_tests()` — proves the wider operand round-trips through dump/load and `AQS_BC_VERSION 2`.
- **D:** import-after-failure retry — register a bad in-memory module + a good one via `aqs_add_module_source`; assert `import bad` fails, then a DIFFERENT good `import` still succeeds (proves the slot was freed and the table is not corrupted). The registry is append-only, so use distinct names rather than same-name replacement.

Gates: `make test-aqs` AND `make test-aqs-gcstress` both green at every stage. Final count ~139.
