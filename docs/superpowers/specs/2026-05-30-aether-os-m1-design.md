...
macOS-style ("Aether") graphical desktop. Not a simulation — an actual kernel
...
| **M5** | Storage & FS | ATA PIO driver, AetherFS (custom on-disk FS), VFS layer, host mkfs tool ✅ |
| **M6** | Userland | GDT/TSS, ring 3, int 0x80 syscalls, ELF64 loader, user paging ✅ |
| **M7** | Graphics | Multiboot2 linear framebuffer, VMM page-mapper, drawing primitives, Aether desktop scene ✅ |
...
shell harness polls the serial log for the `AETHER_BOOT_OK` marker the kernel
prints once it reaches 64-bit C. Found → pass; timeout → fail (dumps serial).

### Success criteria — all met
1. `make` produces `kernel.elf` + `aether.iso` with no errors. ✅
...
3. Injected keystrokes echo on screen ("hello aether os"). ✅
...
- **AetherFS (`fs/aetherfs.c`):** a tiny custom on-disk format — superblock (magic
  "AETHER", version, file count), a 16-entry directory (sectors 1–2), then file
  data from sector 3. Originally read-only; later made read-write (see
  "Writable AetherFS" below).
- **VFS (`fs/vfs.c`):** a one-backend abstraction (`mount/list/size/read`, plus
  `write/delete`) so callers (the ELF loader, apps) don't bind to AetherFS directly.
...
- **User program (`user/`):** `ustart.asm` + `user.c`, linked with `user.ld`
  at 0x40000000, stored on the AetherFS disk as `hello.elf`.
...
- **Finder** → M5: enumerates the AetherFS directory (`vfs_count/ent_name/ent_size`)
...
counters + uptime, and typed text ("hello aether os") all updating concurrently.
...
presents an interactive macOS-style ("Aether") graphical desktop — no third-party
...
- **GUI via syscalls (`include/aether_abi.h`):** create window, clear/rect/text,
  ...
  *surfaces*; the WM composites them. Apps issue these via `user/aether.h`.
...
- **`aetherfs_write`/`aetherfs_delete`** (→ `vfs_write`/`vfs_delete`): create or
...
- **Syscalls:** `SYS_WRITE_FILE` (16) and `SYS_DELETE_FILE` (17), routed through
  `wm_gui_syscall`; userland helpers `write_file`/`delete_file` in `user/aether.h`.
...
## AetherFS v3 — hierarchical inode filesystem (post-M8)
...
- **Kernel (`fs/aetherfs.c`):** mount loads the bitmap + inode table into RAM;
