...
- ABI: `SYS_NET_INFO` (fill {ip,mask,gw,mac}), `SYS_NET_PING(ip)` /
  ...
  `wm_gui_syscall`; helpers in `user/aqua.h`.
...
1. **Kernel boot self-test:** ping gateway 10.0.2.2 and DNS-query 10.0.2.3; on
   both replies print `AQUA_NET_OK` on serial (fits the existing marker test).
...
3. **QMP UI smoke:** drive the Network app to ping the gateway and resolve a
   name; screenshot.
4. `make test` stays green (existing AQUA_BOOT_OK marker).
...
3. **IPv4 + ICMP:** ping the gateway → `AQUA_NET_OK` with RTT.
