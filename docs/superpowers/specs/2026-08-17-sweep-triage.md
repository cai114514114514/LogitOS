...
| `test-live-page` | clicking a link does not run its handler. **Harness explanation ruled out**: the click target is RE-LOCATED from the current screenshot by colour and aimed at the first dark pixel inside the block, with a comment saying why (an earlier step grew the block above it). And the same run's serial shows `LIVE-CSSOM-READ`, `LIVE-CSSOM-WROTE` and `LIVE-MARK-TIMER` -- the page renders and responds; only the link click does not fire |
| `test-browser-https` | **weakest of these, and the only one with a hardcoded rectangle.** `VIEWPORT = (110, 200, 1270, 655)`, and the control's own comment says what its failure means: "if the window never opened, this is not empty for a reason that has nothing to do with the page". But 3902 ink px over 527,800 is 0.74% -- too sparse for wallpaper, more like some text. Between "the window opened somewhere the rectangle does not cover" and "the Browser painted something", and a screenshot settles it in a minute |
