# M21 — AquaScript 自举 + LibAqua（dict → 标准库 → 自举编译器）

## 动机
AquaScript（M20）已经是一门有栈字节码 VM、有 import 模块的小语言（`src/apps/aqs/`，
`/bin/aqs`）。下一步是让它**自举**——用 AquaScript 自己写它的编译器，让 `aqs` 能编译
自己的源码。这是 "from scratch" 项目的成人礼:语言强到能实现自己。

**自举的精确含义(已定)**:**编译器用 AquaScript 写,VM 仍是 C**。VM 是运行时(它定义
...
才好写 **LibAqua 标准库**(阶段 2),有了标准库才好写 **compiler.aqs**(阶段 3)。

```
阶段1 dict  ──►  阶段2 LibAqua  ──►  阶段3 自举编译器
(语言特性)      (用aqs写的stdlib)   (compiler.aqs + .aqc字节码 + 定点测试)
```
...
### 值模型
新对象类型 `O_DICT`(`src/apps/aqs/aqs.h` 的 `ObjType` 加一项),开放寻址哈希表:
```c
...
- 字面量 `{}`、`{"k": v, "k2": v2}`、`{1: "a", 2: "b"}` → 新 `OP_MAKE_DICT n`
  ...
  (注意:`{` 当前未用作语句块——AquaScript 是缩进分块,所以 `{` 自由可用。)
...
### 文件改动(阶段 1)
- `aqs.h`:`O_DICT`、`ObjDict`/`DictEntry`、`IS_DICT/AS_DICT`、`OP_MAKE_DICT`、`OP_IN`、
  `aqs_dict_new/get/set/has/remove/len` 原型。
- `object.c`:dict 对象的分配、哈希探查、rehash、释放(`aqs_free_objects` 加分支)。
- `value.c`:`aqs_print_value` 加 dict 打印(`{k: v, ...}`);dict 相等**按引用(同一对象)**,
...
### 验证(阶段 1)
- host `make test-aqs`:tools/t/aqs_test.c 加 dict 用例 —— 建/读/写/缺键、`.keys/.values/.has/
  .get`、`len`、`k in d`、`for k in d`、int 键、rehash(插 >cap 个键)、覆盖写、删除。
- on-Aqua `make test-aqs-os`:加一个 `dict.aqs` 例子,串口断言输出。
- 不回归:现有 aqs 测试全绿。

---

## 阶段 2 — LibAqua 标准库(设计骨架,落地前补 spec)

用 AquaScript 写、放 `fsroot/aqs/lib/`(或 `/usr/aqs/`)、经 `import` 使用的标准库。范围按
**"够写编译器 + 日常脚本"**裁剪,不堆砌:
...
- `io`:读/写整个文件 —— 需要给 aqs 暴露 open/read/write/close(经 mini-libc fopen 的
  native 桥,或 `syscall(SYS_*)`,A3 已有 syscall 内建)。
- `test`:assert_eq / assert_true + 计数汇总(阶段 3 的自举测试要用)。

风险:LibAqua 自身要有 host 测试(在 host 上跑 .aqs 断言);可能暴露语言 bug,顺带修。

## 阶段 3 — 自举编译器(设计骨架,落地前补 spec)

### 序列化字节码 `.aqc`
当前编译产物是内存里的 `ObjFn`(code 字节 + 常量池),没有落盘格式。自举需要 compiler.aqs
产出 **C VM 能加载执行**的东西 → 定义 `.aqc`:
- 头(magic + 版本)、常量池(int/float/string/嵌套 fn 递归编码)、code 字节、每个 fn 的 arity/
  name/局部数。
- C 侧:`aqs_dump(ObjFn*) -> bytes`、`aqs_load(bytes) -> ObjFn*`;VM 能跑 load 回来的 fn。
- `aqs` CLI:`aqs -c x.aqs -o x.aqc`(只编译、emit 字节码)、`aqs x.aqc`(直接跑字节码)。
  现有 `aqs x.aqs`(编译+跑)不变。

### compiler.aqs
用 AquaScript(+ LibAqua + dict)重写 **lexer + compiler**,emit `.aqc` 字节流(和 C 的
`aqs_dump` 同格式)。VM 不动(仍 C)。

### 自举证明(定点测试)
1. stage0 = 现有 C 的 `aqs`。
2. C-aqs **跑** `compiler.aqs`,让它编译 `compiler.aqs` 自己 → `stage1.aqc`。
3. 用 `aqs stage1.aqc` **跑** stage1(它也是个编译器),再编译 `compiler.aqs` → `stage2.aqc`。
4. **断言 `stage1.aqc == stage2.aqc`(字节级定点)** —— 编译器编译自己产出稳定 → 自举成立。
5. 再抽查:stage1 编译几个 LibAqua/示例,输出与 C-aqs 编译运行一致。
...
- **无 GC**:dict 和编译器都在 arena 上分配,脚本退出回收;长跑 REPL 会泄漏(M20 已知,延后)。
  编译器是一次性进程,可接受。
- **递归深度**:compiler.aqs 的递归下降在 1MB CLI 栈上,深表达式可能压栈 → 必要时调大
  aqs 栈(M18 `exec.c` 的 CLI 栈页数)。
...
## 验证(里程碑级)
- 阶段 1:`make test-aqs`(host)+ `make test-aqs-os`(QEMU)dict 用例全绿,旧用例不回归。
- 阶段 2:LibAqua 各模块 host 断言测试。
