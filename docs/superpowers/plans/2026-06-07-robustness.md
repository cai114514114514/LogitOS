...
# Staged Implementation Plan — AetherScript Robustness Batch

All paths absolute. Each stage MUST build and pass BOTH `make test-as` and `make test-as-gcstress` before the next stage starts. No commit unless the user asks; if needed, branch off `m13-html-css` first.

Gating command (run after every stage):
`cd /Users/wangzhe/ststem && make test-as && make test-as-gcstress`
Expect "all N AetherScript checks passed" (N=135 until Stage 5 adds tests).
...
Gate: `make test-as && make test-as-gcstress` (135 green; behavior unchanged for all existing programs).

---

## Stage 2 — Wide constant index, 16-bit uniform (Item B) — the meaty one

Files: `/Users/wangzhe/ststem/src/apps/as/as.h`, `compiler.c`, `vm.c`

1. `as.h:162`: `#define AS_BC_VERSION 1u` → `2u`.
2. `vm.c:352`: `#define READ_CONST() (frame->fn->consts[READ_BYTE()])` → `(frame->fn->consts[READ_SHORT()])`. (Single line; fixes all 12 reads.)
3. `compiler.c`:
   a. `makeConst` (55-60): delete the `if (k > 255) {...}` block; body becomes `int k = as_chunk_const(current->fn, v); return k;`.
...
Gate: `make test-as && make test-as-gcstress`. The `bc_tests()` roundtrip suite (dump→load→run) proves the wider encoding round-trips; `bc_reject` proves version rejection. 135 green.

---

## Stage 3 — OOM wrapper + sites (Item C)

Files: `/Users/wangzhe/ststem/src/apps/as/object.c`, `vm.c`, `compiler.c`, `lexer.c`, `as.h`

1. `as.h`: add `extern int g_oom;` near the other externs (e.g. after `extern char as_err[256];`, line 236).
2. `object.c`:
   a. Top of file: `int g_oom = 0;` and
      `static void *as_malloc(size_t n){ void *p = malloc(n); if (!p){ snprintf(as_err, sizeof as_err, "out of memory"); g_oom = 1; } return p; }`
      `static void *as_realloc(void *p, size_t n){ void *q = realloc(p, n); if (!q){ snprintf(as_err, sizeof as_err, "out of memory"); g_oom = 1; } return q; }`
      (needs `#include <stdio.h>` for snprintf — add if absent; or use a fixed `strcpy(as_err, "out of memory")`).
   b. Replace `malloc`/`realloc` with wrappers at: alloc_obj (23), as_str_copy (48), as_closure_new (68), as_module_slot (123), as_list_push (148), dict_grow (194), as_chunk_write (271), as_chunk_const (283), gc_mark_obj gray (295).
   c. alloc_obj (23-29): `Obj *o = (Obj*)as_malloc(size); if (!o) return NULL;` then the field writes.
   d. as_str_copy (46-51): `char *buf = as_malloc(len+1); if (!buf) return NULL;` ... then `return as_str_take(buf, len);` (as_str_take calls alloc_obj which may also fail → returns NULL; callers already vary, but the run loop's DISPATCH g_oom check catches it).
   e. as_closure_new (64-74): after `ups = as_realloc/as_malloc(...)`, `if (!ups) return NULL;` before the loop.
   f. as_module_slot (121-124): after the grow realloc, `if (g_oom) return NULL;` before writing `m->vars[...]`.
   g. as_list_push (144-151): after the grow realloc, `if (g_oom) return;` before `l->items[l->count++] = v;`.
   h. dict_grow (191-206): after `ne = as_malloc(...)`, `if (!ne) return;` before memset/rehash.
   i. as_chunk_write (267-274): after the realloc, `if (g_oom) return;` before `fn->code[fn->count++] = b;`.
   j. as_chunk_const (276-287): after the realloc, `if (g_oom) return fn->kcount;` (or return 0) before the store; the compiler's post-loop g_oom check handles the failure.
   k. gc_mark_obj (289-298): rewrite the grow block:
      `if (gray_count + 1 > gray_cap) { int nc = gray_cap < 16 ? 16 : gray_cap * 2; Obj **ng = (Obj**)as_realloc(gray, (size_t)nc * sizeof(Obj*)); if (!ng) { o->marked = 0; g_oom = 1; return; } gray = ng; gray_cap = nc; } gray[gray_count++] = o;`
3. `vm.c`:
   ...
   c. str_concat (103-109): `char *buf = (char*)as_malloc((size_t)n+1); if (!buf) return NULL;` and at op_ADD (vm.c:410) where it's used: `ObjStr *s = str_concat(...); if (!s) goto err;` then `sp -= 2; push(OBJ_VAL(s));`.
   d. module_source (134, 148, 150): use as_malloc/as_realloc; the existing `if (buf)` (135) and the `!src` caller check (338) cover NULL. For the file path add `if (!buf) { fclose(f); return NULL; }` after line 148.
   e. runtime_error (90-96) + ensure_exc (85-88): guard the as_str_copy result — `ObjStr *s = as_str_copy(...); return throw_value(s ? OBJ_VAL(s) : NIL_VAL);` and in ensure_exc `if (!g_has_exc) { ObjStr *s = as_str_copy(as_err, (int)strlen(as_err)); g_exc = s ? OBJ_VAL(s) : NIL_VAL; g_has_exc = 1; }`.
   f. Note: `as_malloc`/`as_realloc` are static in object.c; str_concat/module_source live in vm.c and can't see them. Resolution: either (i) make the two wrappers non-static and declare them in `as.h`, OR (ii) keep vm.c's two mallocs using raw malloc but set g_oom + as_err manually on NULL. CHOICE: expose them — add `void *as_malloc(size_t);` `void *as_realloc(void*, size_t);` to as.h, drop `static` in object.c. Cleaner and reused by lexer.c/compiler.c too.
4. `compiler.c`:
   a. string() (179): `char *buf = (char*)as_malloc((size_t)t.len+1); if (!buf) { error("out of memory"); return; }`.
   b. as_compile_module (775+): set `g_oom = 0;` near `had_error = 0;` (784); after the parse loop (799) / before `return had_error ? NULL : script;` (805) add `if (g_oom) return NULL;` (or fold: `return (had_error || g_oom) ? NULL : script;`).
5. `lexer.c`:
   a. push() (13-21): `b->t = (Token*)as_realloc(b->t, b->cap * sizeof(Token)); if (!b->t) return;` (guards the deref on 19). Since push is void, also have `as_lex` poll `g_oom`: in the scan loop add a check that breaks to the existing error-exit (the `err`/`free(b.t); return NULL` path) when `g_oom` is set, so a NULL token buffer never gets indexed. Confirm by reading lexer.c's `as_lex` body (lines ~23-150) for the exact `err`/return-NULL site and mirror it.

Gate: `make test-as && make test-as-gcstress`. 135 green (no new tests — see spec testability note). gcstress is especially important here: it collects before every alloc, exercising the gray-stack path and the wrapper plumbing under load.

---

## Stage 4 — Import-failure cleanup (Item D)

File: `/Users/wangzhe/ststem/src/apps/as/vm.c`, `as_import` (328-345)
...
Gate: `make test-as && make test-as-gcstress` (135 green).

---

## Stage 5 — Tests (Item T)

File: `/Users/wangzhe/ststem/tools/t/as_test.c`
...
5. **D — import retry:** in `main`, alongside the existing `as_add_module_source("mathx", MATHX)` (line 204), register a deliberately-failing module and a good one, then:
...
Gate (final): `make test-as && make test-as-gcstress` — expect "all ~139 AetherScript checks passed" on BOTH. Then optionally `make test-as-os` to confirm on-Aether examples still run.
...
- AS_BC_VERSION bump is mandatory and the ONLY .la format change.
- No public AetherScript behavior changes for any program that previously ran; the only observable new behavior is errors (overflow / OOM) replacing crashes, and >255-const modules compiling.
