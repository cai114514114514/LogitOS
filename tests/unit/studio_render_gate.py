...
mutations=[('studio_render.inc','case ST_INK_KEYWORD:return rgb(198,120,221);','case ST_INK_KEYWORD:return AUI_TEXT;','highlight'),
           ...
           ('studio_completion.inc','int index = popup.first + (y - popup.y) / ST_ROW;','int index = 0;','popup scrolled inserts'),
           ('studio_completion.inc','st_engine_dismiss_completion(engine);','/* mutant: outside click retains popup */','popup outside dismisses')]
...
for filename,old,new,label in mutations:
    ...
    with tempfile.TemporaryDirectory(prefix='studio-render-') as tmp:
        ...
        sources=list((ROOT/'c/apps/studio').glob('*.c'))+[ROOT/x for x in ('c/apps/as/editor/completion.c','c/apps/as/common/version.c','c/lib/agent/json.c','c/lib/agent/task.c','c/drivers/block/crc32.c')]
