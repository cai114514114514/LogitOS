...
flags=['clang','-std=c11','-O2' if a.benchmark else '-O1','-g','-Wall','-Wextra','-Werror','-pthread','-DMM_HOSTTEST','-DMM_CONCURRENT','-DLOGIT_LOCK_HOST','-Itests/unit/mmstub','-Ic/kernel/mm']
if not a.benchmark:flags+=['-fsanitize=address,undefined','-fno-sanitize-recover=all']
sources=['tests/unit/bkl_kheap_test.c','c/kernel/mm/kheap.c','c/kernel/cpu/spinlock.c'];report=[]
...
if a.baseline_tar:
 ...
 old=out/'before/c/kernel/mm/kheap.c';old.parent.mkdir(parents=True,exist_ok=True)
 ...
 with tarfile.open(a.baseline_tar,'r:gz') as archive:
  old.write_bytes(archive.extractfile('c/kernel/mm/kheap.c').read())
 weak.write_bytes((root/'include/weaksym.h').read_bytes())
 measure_flags=['clang','-std=c11','-O2','-g','-Wall','-Wextra','-Werror','-pthread','-DMM_HOSTTEST','-DMM_CONCURRENT','-DLOGIT_LOCK_HOST','-DKHEAP_MEASURE_ONLY','-Itests/unit/mmstub','-Ic/kernel/mm']
 bins={}
 for name,allocator in [('before',old),('after',root/'c/kernel/mm/kheap.c')]:
 ...
 result['source_sha256']={name:hashlib.sha256(path.read_bytes()).hexdigest() for name,path in [('before',old),('after',root/'c/kernel/mm/kheap.c'),('shared_ticket_lock',root/'c/kernel/cpu/spinlock.c')]}
