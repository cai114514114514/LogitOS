...
/* ---------------------------------------------------- the buffered layer --
 *
 * THE BUG THIS EXISTS FOR. Our TLS is a layer between http1.c and the socket,
 * and tls_rec_pull() drains the WHOLE TCP receive buffer into the session's
 * own record buffer before handing back ONE record's plaintext. So after the
 * first read the socket reports `tcp_available() == 0` and `tls_pending() ==
 * 0` -- SOCK_P_READABLE clear -- while several complete records still sit in
 * the session, undecrypted and invisible to anything that asks the socket.
 *
 * h1_conn_pump used to skip its read whenever the transport's poll hint said
 * "not ready". Against that TLS layer the hint is a lie, and the result was
 * measured: the Browser parsed www.baidu.com's status line and then sat for
 * SIXTY SECONDS on a response that had already arrived in full, until the
 * server's close_notify put one more byte on the wire and flipped the bit.
 * The kernel's own client never saw this because it has no poll at all -- it
 * simply calls tls_recv in a loop, which is the behaviour this restores.
 *
 * `bufrec` is that transport: `poll` reports only what is on the WIRE, while
 * `read` also drains an internal record buffer the poll cannot see. Both the
 * assertions below fail on the pre-fix parser -- one on the contract (it must
 * not ask about reading at all) and one on the outcome (the message must
 * complete without another wire event). */
struct bufrec {
    const char *wire;          /* what the "server" sent, all at once */
    int   wire_len, wire_off;  /* how much has been pulled into `buf` */
    char  buf[4096];           /* the layer's record buffer: invisible to poll */
    int   buf_len, buf_off;
    int   record;              /* bytes handed back per read: one "record" */
    int   arrived;             /* the wire delivery has happened */
    int   read_polls;          /* times poll was asked about READING */
    int   reads;
};

static int bufrec_read(void *ctx, void *buf, int len)
{
    struct bufrec *b = (struct bufrec *)ctx;
    b->reads++;
    if (b->buf_off >= b->buf_len) {
        /* Nothing buffered: pull everything the wire has, exactly as
         * tls_rec_pull() empties the TCP receive buffer in one go. */
        if (!b->arrived || b->wire_off >= b->wire_len) return H1_AGAIN;
        int take = b->wire_len - b->wire_off;
        if (take > (int)sizeof b->buf) take = (int)sizeof b->buf;
        memcpy(b->buf, b->wire + b->wire_off, (size_t)take);
        b->wire_off += take;
        b->buf_len = take; b->buf_off = 0;
    }
    int take = b->buf_len - b->buf_off;
    if (b->record > 0 && take > b->record) take = b->record;
    if (take > len) take = len;
    memcpy(buf, b->buf + b->buf_off, (size_t)take);
    b->buf_off += take;
    return take;
}

static int bufrec_write(void *ctx, const void *buf, int len)
{
    (void)buf;
    struct bufrec *b = (struct bufrec *)ctx;
    b->arrived = 1;                       /* the request went out; the reply lands */
    return len;
}

/* The socket's view. It can see the wire and nothing else -- which is exactly
 * what sock_poll() can see, and exactly why it must not gate a read. */
static int bufrec_poll(void *ctx, int want_write)
{
    struct bufrec *b = (struct bufrec *)ctx;
    if (!want_write) { b->read_polls++; return b->wire_off < b->wire_len ? 1 : 0; }
    return 1;
}

static void t_conn_layered_transport(void)
{
    ...
    struct bufrec b;
    memset(&b, 0, sizeof b);
    b.wire = wire; b.wire_len = (int)sizeof wire - 1;
    b.record = 20;                        /* several records, one wire delivery */

    struct h1_transport t;
    t.read = bufrec_read; t.write = bufrec_write; t.poll = bufrec_poll; t.ctx = &b;

    struct h1_conn c;
    char *req = strdup("GET / HTTP/1.1\r\nHost: x\r\n\r\n");
    OK(h1_conn_start(&c, &t, req, (int)strlen(req)) == H1_OK);

    /* A bounded pump budget, because "hangs forever" must be a FAILURE and not
     * a test that never returns. 200 is far more than the ~9 records here. */
    int guard = 0;
    while (h1_conn_pump(&c) < H1_C_DONE && ++guard < 200) { }

    OK(c.state == H1_C_DONE);
    OK(body_is(&c.resp, "hello world"));
    OK(c.resp.code == 200);
    /* The contract, asserted directly: the read direction is never polled. A
     * hint may be believed when it says ready; it may never be believed when
     * it says not ready. */
    OK(b.read_polls == 0);
    /* And the whole message came out of ONE wire delivery -- no second arrival
     * was needed to shake the rest loose, which is precisely what the bug
     * required and what made it look like a dead connection. */
    OK(b.wire_off == b.wire_len);
    h1_conn_free(&c);
}

/* h1_conn_progress is the counter a pumping caller uses to decide when to stop
 * draining -- browser_rt.c's loop runs on it, because "the socket says it is
 * no longer readable" is the same lie as above. So its two properties are
 * load-bearing: a pump that moves bytes raises it, one that moves none leaves
 * it exactly where it was. */
static void t_conn_progress(void)
{
    ...
    struct h1_transport t;
    t.read = bufrec_read; t.write = bufrec_write; t.poll = NULL; t.ctx = &b;
    struct h1_conn c;
    char *req = strdup("GET / HTTP/1.1\r\nHost: x\r\n\r\n");
    OK(h1_conn_start(&c, &t, req, (int)strlen(req)) == H1_OK);

    OK(h1_conn_progress(&c) == 0);
    int moved = 0, stalled = 0, guard = 0;
    for (;;) {
        unsigned long long before = h1_conn_progress(&c);
        int st = h1_conn_pump(&c);
        unsigned long long after = h1_conn_progress(&c);
        OK(after >= before);
        if (after > before) moved++; else stalled++;
        if (st >= H1_C_DONE || ++guard > 200) break;
    }
    OK(c.state == H1_C_DONE && body_is(&c.resp, "abcde"));
    /* Every pump before completion moved bytes: the drain loop would not have
     * stopped early, which is the property browser_rt.c relies on. */
    OK(stalled == 0);
    OK(moved >= 2);
    OK(h1_conn_progress(&c) == (unsigned long long)((int)strlen("GET / HTTP/1.1\r\nHost: x\r\n\r\n") + b.wire_len));
    h1_conn_free(&c);
}
