...
int main(void)
{
    ...
    g_ctx = js_page_ctx();
    FILE *f=fopen("tests/fixtures/engine-expansion/digest-vectors.js","rb");
    if(!f)return 2;
    char source[16384];size_t len=fread(source,1,sizeof source,f);fclose(f);
    if(len==sizeof source)return 2;
    JSValue v=JS_Eval(g_ctx,source,len,"digest-vectors.js",JS_EVAL_TYPE_GLOBAL);
    ...
    ckjs("digestTestDone", "digest promises terminate");
    ckjs("digestTestResults.length===50", "all vector and rejection cases ran");
    ckjs("digestTestResults.every(function(x){return x[1]})", "real SHA digest bytes match independent vectors");
    const char *expr="JSON.stringify(digestTestResults.filter(function(x){return !x[1]}))";
    ...
    printf("web-digest: %d checks, %d failures\n",checks,fails);return fails?1:0;
}
