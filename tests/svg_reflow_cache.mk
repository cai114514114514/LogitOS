...
$(SVG_REFLOW_DIR)/current: $(SVG_REFLOW_DEP)
	...
	@$(CC) -O2 -w $(BTEST_INC) $(CSS_INC) $(KMM_INC) -o $@ $(sort $(SVG_REFLOW_SRC) $(HTML_PARSER_SRC)) $(BUILD)/libcss_host.a $(RUST_LIB_HOST) -lm
$(SVG_REFLOW_DIR)/old: $(SVG_REFLOW_DEP)
	@mkdir -p $(SVG_REFLOW_DIR)
	@$(CC) -O2 -w -DLAYOUT_SVG_NO_REFLOW_CACHE $(BTEST_INC) $(CSS_INC) $(KMM_INC) -o $@ $(sort $(SVG_REFLOW_SRC) $(HTML_PARSER_SRC)) $(BUILD)/libcss_host.a $(RUST_LIB_HOST) -lm
$(SVG_REFLOW_DIR)/small: $(SVG_REFLOW_DEP)
	@mkdir -p $(SVG_REFLOW_DIR)
	@$(CC) -O2 -w -DSVG_REFLOW_MAX=2 $(BTEST_INC) $(CSS_INC) $(KMM_INC) -o $@ $(sort $(SVG_REFLOW_SRC) $(HTML_PARSER_SRC)) $(BUILD)/libcss_host.a $(RUST_LIB_HOST) -lm
$(SVG_REFLOW_DIR)/key-bound: $(SVG_REFLOW_DEP)
	@mkdir -p $(SVG_REFLOW_DIR)
	@$(CC) -O2 -w -DSVG_REFLOW_KEYS=1 -DSVG_REFLOW_PASSIVE_KEYS=1 $(BTEST_INC) $(CSS_INC) $(KMM_INC) -o $@ $(sort $(SVG_REFLOW_SRC) $(HTML_PARSER_SRC)) $(BUILD)/libcss_host.a $(RUST_LIB_HOST) -lm
$(SVG_REFLOW_DIR)/san: $(SVG_REFLOW_DEP)
	@mkdir -p $(SVG_REFLOW_DIR)
	@$(CC) -O1 -g -w -fsanitize=address,undefined -fno-omit-frame-pointer $(BTEST_INC) $(CSS_INC) $(KMM_INC) -o $@ $(sort $(SVG_REFLOW_SRC) $(HTML_PARSER_SRC)) $(BUILD)/libcss_host.a $(RUST_LIB_HOST) -lm
