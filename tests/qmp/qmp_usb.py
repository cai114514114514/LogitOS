...
# SIGTERM from the parent timeout must unwind this scope, just like an
# assertion failure or Ctrl-C. SIGKILL would strand the wrapper's QEMU child.
def terminate_test(signum, frame):
    raise SystemExit(128 + signum)

previous_term = signal.signal(signal.SIGTERM, terminate_test)
proc = ser = qmp = qf = None
try:
