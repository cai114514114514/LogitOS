...
typedef struct {char path[128],name[64];int depth,directory,expanded;} StFile;
typedef struct {StFile files[ST_FILES];int count,limited;char path[128];} StListing;
typedef struct {int start,end,line,column;char message[256],code[24];} StProblem;
...
enum StMove {ST_LEFT,ST_RIGHT,ST_HOME,ST_END,ST_UP,ST_DOWN,ST_FIRST,ST_LAST};
