...
typedef struct {
    char label[64];          /* includes every valid A3 identifier */
    char insert[64];
    int  kind;               /* CMP_*                         */
...
/* The editor uses the same keyword/builtin vocabulary as completion. */
int as_word_kind(const char *word, int length);
...
/* Per-request providers keep separate Studio projects from sharing a mutable
 * global module lookup. Calls remain serialized: tokenizer scratch is reused. */
typedef struct {
    void *context;
    int (*list)(void *,char names[][48],int max);
    int (*read)(void *,const char *name,char *buf,int max);
} CmpProviders;
...
/* Lexical context only; A3 semantic queries resolve the receiver separately. */
