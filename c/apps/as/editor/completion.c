 ...
 * its own tolerant tokenizer (the production frontend/lexer.c uses snprintf and aborts on
...
/* Mirrors the as_define_native() calls in legacy/vm.c + legacy/builtins.c + legacy/ports.c.
...
static const char *const BUILTINS[] = {
    ...
    "parse_int","wrapping_add","wrapping_sub","wrapping_mul","wrapping_shl",
...
/* the M23.5 system surface (mirrors legacy/builtins.c) -- prefix-typed (SYS_/EV_), so
 * they only surface when the user starts typing one */
/* The checker and completion must offer the same system identities. */
static const char *const SYSCONSTS[] = {
#define AS_SYSTEM_CONSTANT(name, value) #name,
...
/* M27 ports (mirrors legacy/ports.c's port_method/proc_method). */
...
int as_word_kind(const char *s, int n){
    if (is_kw(s,n)) return CMP_KEYWORD;
    for (int i=0; BUILTINS[i]; i++) if (c_neq(s,BUILTINS[i],n)) return CMP_BUILTIN;
    return CMP_LOCAL;
}
...
/* ---- the dispatcher ---- */
int as_complete_with(const char *src, int len, int caret, Completion *out, int max,const CmpProviders *providers){
    ...
    if (cx.after_import && cx.receiver[0]==0) {
        char names[64][48];
        int nm = providers&&providers->list ? providers->list(providers->context,names,64) : 0;
        for (int i=0;i<nm && na<512;i++) put(all,&na,512,names[i],c_slen(names[i]),CMP_MODULE,90);
        goto rank;
    }
    if (cx.receiver[0] && providers && providers->read && is_imported(toks, nt, src, cx.receiver)) {
        static char mbuf[65536];
        int ml = providers->read(providers->context,cx.receiver, mbuf, (int)sizeof mbuf);
...
CmpCtx as_completion_context(const char *source,int length,int caret)
{return ctx_at(source,length,caret);}

static int legacy_list(void *context,char names[][48],int max)
{(void)context;return g_list?g_list(names,max):0;}
static int legacy_read(void *context,const char *name,char *buf,int max)
{(void)context;return g_read?g_read(name,buf,max):-1;}
int as_complete(const char *src,int len,int caret,Completion *out,int max)
{CmpProviders p={0,g_list?legacy_list:0,g_read?legacy_read:0};return as_complete_with(src,len,caret,out,max,&p);}
