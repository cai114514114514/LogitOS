...
static void push(TokBuf *b, TokType ty, const char *start, int len, int line)
{
    if (b->n + 1 > b->cap) {
        ...
        b->t = (Token *)as_realloc(b->t, b->cap * sizeof(Token));
        if (g_oom) return;                  /* as_lex polls g_oom -> bails to its err exit */
    }
...
Token *as_lex(const char *src, int *count)
{
    ...
    while (*p) {
        ...
        if (bracket == 0) {
            ...
            if (*p == 0) break;
            if (col > indent[ni - 1]) {
                if (ni >= 64) { snprintf(as_err, sizeof as_err, "too many indentation levels (line %d)", line); err = 1; break; }
                indent[ni++] = col; push(&b, T_INDENT, ls, 0, line);
            } else {
                while (col < indent[ni - 1]) { ni--; push(&b, T_DEDENT, p, 0, line); }
                if (col != indent[ni - 1]) { snprintf(as_err, sizeof as_err, "inconsistent indentation (line %d)", line); err = 1; break; }
            }
        }
    ...
    if (err || g_oom) { free(b.t); return NULL; }   /* g_oom: as_err already "out of memory" */
