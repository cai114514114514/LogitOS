...
int x509_verify_signed_by(const struct cert *child, const struct cert *issuer)
{
    ...
    TLSPROF_END(tls_chain_sig);
    return ok ? X509_OK : X509_E_SIG;
}
...
static int signed_by_root(const struct cert *top)
{
    ...
    for (int i = 0; i < logit_nroots && !found; i++) {
        ...
        if (rsa) {
            ...
            if (verify_with_key(top, KEY_EC, r->curve, r->ec, r->eclen, 0, 0, 0, 0)) found = 1;
        }
    }
    TLSPROF_END(tls_root_search);
    return found;
}
