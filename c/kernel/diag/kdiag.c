...
void idt_install_gate(int vec, void *handler);   /* c/kernel/cpu/irq/idt.c */
...
static int render_kstat(char *buf, int max)
{
    ...
    /* Non-zero means some entry path calls into the kernel with a stack the
     * ABI says cannot be misaligned. kprintf survives it; the caller is still
     * wrong, and the address is what makes it findable in build/kernel.map. */
    n += ksnprintf(buf + n, max - n, "abi_misaligned     %llu\n",
