...
uint64_t swap_poll_cycles(void);      /* cycles spent actively polling the device */
uint64_t swap_poll_worst(void);       /* worst single transfer, cycles */
 ...
 * runs past SWAP_POLL_BUDGET_CYC gives the lock back and keeps going. Reporting
...
uint64_t swap_poll_parks(void);    /* times a transfer parked while waiting */
