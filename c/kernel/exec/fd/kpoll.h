 ...
 * sched.h), and c/kernel/exec/fd/file.c carries the path-qualified include that is
 ...
 * struct waiter's poll-hook comment in c/kernel/sync/wait.h). Reading first and
...
#include "kernel/sync/wait.h"   /* struct waitq / struct waiter. Path-qualified
 ...
 * argument c/kernel/sync/wait.c makes for putting `struct waiter` there. */
...
struct poll_table {
     ...
     * 2 of c/kernel/sync/wait.h, with this table standing in for the object. */
    spinlock_t      lock;
    int             triggered;
    int             n;
    int             overflow;
