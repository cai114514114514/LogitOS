                                ...
                                char **envp, int envc, int stack_pages)
{
     ...
     *     is faulted in by c/kernel/mm/virt/fault.c's do_anon(), which maps from the
...
static int spawn_initial(const char *path, char **argv, int service)
{
     ...
     * (c/kernel/cpu/irq/interrupts.c's non-preemptible busy flags). Opening it
     ...
     * schedule() (c/kernel/cpu/irq/interrupts.c:288), and without that the sti
