 ...
 *
 * The vendor-wide display path lives in amd/bootfb.c. The automatic probe
 * retains a read-only Polaris10 snapshot until the platform supplies verified
 * firmware and an exclusive VRAM reservation record to polaris/desktop.c.
 * That separate entry composes firmware/SMU/SDMA and installs its own presenter
 * after execution readbacks. The automatic native command path is RV100
