/* c/lib/video/h264_cabac_tables.h -- CABAC context initialisation constants.
 *
 * ITU-T H.264 Tables 9-12 through 9-33: the (m, n) pair of each context
 * variable, for I slices and for each of the three cabac_init_idc values a
 * P or B slice may signal. Spec 9.3.1.1 turns a pair into an initial state:
 *
 *     preCtxState = Clip3(1, 126, ((m * Clip3(0, 51, SliceQPY)) >> 4) + n)
 *     preCtxState <= 63 -> pStateIdx = 63 - preCtxState, valMPS = 0
 *     otherwise         -> pStateIdx = preCtxState - 64, valMPS = 1
 *
 * ctxIdx 0..459 only. 460..1023 are the field-coded (MBAFF / field picture)
 * duplicates of the residual contexts, and this decoder is frame-coded 4:2:0
 * 8-bit -- frame_mbs_only_flag == 0 is refused in the SPS, so a stream can
 * never reach them. The 8x8-transform contexts a High-profile stream does
 * need (transform_size_8x8_flag 399..401, the ctxBlockCat 5 residual
 * 402..435) are inside this range. 4:2:0 8x8 luma blocks carry no
 * coded_block_flag at all (7.3.5.3.3 sends it only when maxNumCoeff != 64),
 * which is why ctxIdx 1012..1015 is absent and not missing.
 *
 * These are normative constants, so the only question is transcription. They
 * were agreed on by THREE independent implementations: two that hold (m, n)
 * pairs, and one that holds the other representation -- the resolved
 * (pStateIdx, valMPS) for every QP, which checks 9.3.1.1's derivation as well
 * as the constants. 1787 pair comparisons and 92924 derived-state comparisons,
 * zero disagreements. (53 (ctxIdx, slice type) combinations the spec leaves
 * undefined -- P-slice mb_type contexts in an I slice and the like -- are
 * excluded: the sources fill them differently and nothing reads them.)
 * tests/unit/h264_cabac_test.c pins the 9.3.1.1 derivation itself.
 *
 * Generated, not hand-typed. Do not edit: fix the source and regenerate.
 */
#ifndef LOGIT_H264_CABAC_TABLES_H
#define LOGIT_H264_CABAC_TABLES_H

#include <stdint.h>

#define H264_NCTX 460

/* I slices (Table 9-12 .. 9-33, the cabac_init_idc-independent column) */
static const int8_t h264_cabac_init_I[H264_NCTX][2] = {
...
/* P and B slices, indexed by cabac_init_idc 0..2 */
static const int8_t h264_cabac_init_PB[3][H264_NCTX][2] = {
{
    {   20,  -15 }, {    2,   54 }, {    3,   74 }, {   20,  -15 },
    {    2,   54 }, {    3,   74 }, {  -28,  127 }, {  -23,  104 },
...
#endif /* LOGIT_H264_CABAC_TABLES_H */
