...
/* Longest cursive run we compute joining forms for. A line that reaches this
 * many code points of one script does not fit any window we can draw; the
 * limit exists so the joining buffer can live on the stack, and exceeding it
 * is reported rather than silently truncated into wrong letter forms. */
#define SH_MAX_JOIN      1024
              ...
              int script, int rtl, struct shape_glyph *buf, int cap)
{
    ...
    uint8_t forms[SH_MAX_JOIN];
    if (plan.cursive) {
        if (n > SH_MAX_JOIN) return -1;
        script_arabic_joining(cps, n, forms);
    }
    ...
    for (int i = 0; i < n; i++) {
        ...
        gid = glyph_of(f, cp);
        if (plan.cursive && forms[i] < AJ_NFORM)
    ...
    if (plan.has_gpos && plan.n_gpos_lk > 0) {
         ...
         * ancestors, most Type-1 ports. Marks are stepped over (the table
         * kerns letters, not the marks between them), and the pair's cursor
         * lands ON the second glyph so "AVA" kerns twice.
         *
         * The value is SPLIT, half onto each glyph: the first glyph's advance
         * takes kern >> 1 and the second takes the remainder as BOTH an advance
         * and an x offset. Putting it all on the first glyph gives the same
         * total width and the wrong glyph positions, which is exactly the kind
         * of difference only a glyph-by-glyph differential catches. */
        for (int i = 0; i < c.n; ) {
            int j = i + 1;
            while (j < c.n && (c.g[j].props & SH_P_MARK)) j++;
            if (j >= c.n) break;
            if (c.g[i].props & SH_P_MARK) { i = j; continue; }
            int kern = otl_kern_pair(&plan.kern, c.g[i].gid, c.g[j].gid);
            if (kern) {
                int k1 = kern >> 1;            /* arithmetic, so it floors */
                int k2 = kern - k1;
                c.g[i].x_advance += k1;
                c.g[j].x_advance += k2;
                c.g[j].x_offset  += k2;
