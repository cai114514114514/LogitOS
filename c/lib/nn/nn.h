...
/* Rotary position embedding, applied in place to one head's q or k vector of
 * `n` values (n even) at position `pos`, with the usual theta base. Rotates
 * the pair (x[2i], x[2i+1]), which is the INTERLEAVED convention llama2.c and
 * the original paper use -- NOT the split-half convention huggingface's
 * implementation uses. The two produce different numbers from the same
 * weights, so the convention is named here rather than left to be discovered
 * by a model that outputs plausible nonsense. */
