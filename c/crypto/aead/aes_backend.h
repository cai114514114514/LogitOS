...
/* The three primitives AES-GCM is built from, behind function pointers so a
 * faster/constant-time implementation can be selected once at boot.
 *
 * The split is deliberate: everything ABOVE these three (CTR mode, the GHASH
 * driver loop, the J0 construction, the tag comparison, the decrypt-after-
 * authenticate ordering) stays in ONE place in aesgcm.c and is shared by every
 * backend. A backend cannot get the mode wrong, only the primitive -- and the
 * primitive is exactly what a differential test can cross-check block by
 * block. A second full GCM implementation would have doubled the amount of
