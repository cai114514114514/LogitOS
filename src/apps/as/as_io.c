 ...
 * set a capture buffer via as_capture() so they can assert on `print` output
 * in-process; the same as_emit then appends there instead of writing. */
static char *g_cap = NULL;
static int   g_cap_len = 0, g_cap_cap = 0;

void as_capture(char *buf, int cap)   /* buf==NULL restores normal stdout output */
{
