/* aqs_bc.c -- the .la compiled-bytecode (de)serializer.
 *
 ...
 * aqs_dump serializes that tree to a FILE; aqs_load rebuilds it from a byte
 ...
 * cap/kcap (rebuilt = count/kcount), ObjStr->hash (recomputed by aqs_str_copy),
 ...
 * written only as metadata (so aqs_closure_new can size the upvalue array). */
#include "aqs.h"
#include <stdio.h>
#include <string.h>
#include <stdlib.h>

/* file format: header "LAQ1" + u32 AQS_BC_VERSION, then a recursive ObjFn:
    ...
    for (int i = 0; i < fn->kcount; i++) {
        ...
        case V_OBJ:
            if (IS_STR(v)) {
                ...
                strcpy(aqs_err, "aqs_dump: unserializable constant");
                return 1;
            }
            break;
        default:
            strcpy(aqs_err, "aqs_dump: unknown value type");
            return 1;
        }
    }
    return 0;
...
int aqs_dump(ObjFn *fn, FILE *out)
{
    ...
    if (wr_u32(out, AQS_BC_VERSION)) return 1;
 ...
 * aqs_free_objects/GC (the load is abandoned). */
static ObjFn *load_fn(Cursor *c, int *err)
{
    ObjFn *fn = aqs_fn_new();          /* code/consts NULL, count/kcount 0, module NULL */
    uint32_t u;
    ...
    if (name_len) {
        ...
        fn->name = aqs_str_copy(nm, (int)name_len);   /* recomputes hash */
        free(nm);
    }
    ...
    for (uint32_t i = 0; i < kcount; i++) {
        ...
        case K_STR: {
            ...
            fn->consts[i] = OBJ_VAL(aqs_str_copy(sb ? sb : "", (int)slen));
            free(sb);
            break;
        }
        case K_FN: {
        ...
        default:
            strcpy(aqs_err, "aqs_load: bad constant tag");
            *err = 1;
            return fn;
        }
    }
...
ObjFn *aqs_load(const uint8_t *buf, int len)
{
    ...
    if (ver != AQS_BC_VERSION) return NULL;

    Cursor c = { buf, 8, len };
    aqs_gc_push_disable();             /* the tree is unrooted until returned (mirrors aqs_compile) */
    int err = 0;
