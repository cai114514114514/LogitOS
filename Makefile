...
# The negative control for the fuzz harness itself. IMG_SABOTAGE injects a
# one-line out-of-bounds read into the GIF decoder; the fuzzer MUST fail.
# A fuzz target that cannot fail is a green light wired to nothing -- the
# audio line found exactly that in its own harness today.
test-img-fuzz-negctl: $(RUST_LIB_HOST)
	@mkdir -p $(BUILD)/imgstill
	@python3 tests/unit/img_still_gen.py $(BUILD)/imgstill >/dev/null
	@$(CC) -O1 -g -DIMG_SABOTAGE=1 -fsanitize=address,undefined -fno-sanitize-recover=all \
	    -o $(BUILD)/img_fuzz_sab tests/unit/img_fuzz.c \
	    $(IMG_HOST_SRC) $(RUST_LIB_HOST) $(IMG_HOST_INC)
	@if ASAN_OPTIONS=detect_leaks=1:abort_on_error=1 UBSAN_OPTIONS=halt_on_error=1 \
	      $(BUILD)/img_fuzz_sab 4000 $(BUILD)/imgstill >$(BUILD)/img_fuzz_sab.log 2>&1; then \
	    echo "NEGATIVE CONTROL FAILED: sabotaged decoder fuzzed clean"; exit 1; \
	else \
	    echo "negative control ok: sabotage caught ($$(grep -m1 -oE 'ERROR: [A-Za-z]+' $(BUILD)/img_fuzz_sab.log || echo aborted))"; \
	fi

# Everything above, in one go.
test-img: test-png test-jpeg test-svg test-img-still test-img-anim test-img-exif test-img-fuzz test-img-fuzz-negctl
