...
# --- A SECOND ROOT, so the fixtures can see which table was consulted -------
#
# The three fixtures above are built from the same objects and cover
# accept/reject/foreign-signer, and with PKG_ROOT_N == 1 they pass whether
# lpk_verify reads the compiled-in PKG_ROOTS array or goes through
# pkg_root_count()/pkg_root_key() -- the two name the same 32 bytes, so a
# one-root gate says nothing about the accessors the header promises are the
# single point of change (pkgsig.h) and that /bin/pkgverify --roots already
# uses. That is the whole reason this rule exists.
#
# So: a SCRATCH trust set with two roots, built into $(BUILD) and shipped
# nowhere -- tools/pkgroots/ is not touched and no ISO ever sees it. The second
# root is the foreign signer, taken out of build/foreign.lpk's own signer field
# (bytes 128..159) rather than written down here, so the scratch root is by
# construction the key that signed the fixture and cannot drift from it. It is
# named to sort SECOND, because a verifier that compares entry 0 and stops is
# exactly what a one-entry table lets you write, and only a root at index 1
# catches it.
#
# The gate runs the same test binary twice -- once against the ordinary one-root
# include expecting foreign.lpk REFUSED, once against the scratch two-root
# include expecting it ACCEPTED at index 1. The flip is observed, not assumed.
.PHONY: test-pkg-roots test-pkg-poison

LPK_ROOTS2      := $(BUILD)/pkgroots2
LPK_TEST_SRC    := tests/unit/pkg_roots_test.c c/crypto/pubkey/ed25519.c \
                   c/crypto/hash/sha256.c c/crypto/hash/sha384.c
LPK_TEST_INC    := -Ic/crypto -Ic/crypto/trust
LPK_TEST_CF     := -O1 -g -Wall -Wextra -fsanitize=address,undefined -fno-sanitize-recover=all

# pkgsig.c is COPIED beside the scratch include rather than compiled in place
# with a -I in front, and the first attempt at this rule did the latter and
# silently tested nothing: `#include "pkgroots.inc"` is the quoted form, which
# searches the INCLUDING FILE'S OWN DIRECTORY before any -I, so c/crypto/trust's
# one-root include won and the scratch build reported one root. The gate caught
# it -- it asserts pkg_root_count() == 2 before it asserts anything about a
# verdict, precisely so a scratch trust set that failed to take cannot look like
# a passing test. The copy is a byte-for-byte cp with a dependency on the
# original, so what is compiled is what is in the tree.
#
# od, not xxd: coreutils is present wherever this builds, vim's tools are not.
$(LPK_ROOTS2)/pkgroots.inc: $(BUILD)/foreign.lpk tools/genpkgroots.py $(wildcard tools/pkgroots/*.pub)
	@mkdir -p $(LPK_ROOTS2)/keys
	@cp tools/pkgroots/*.pub $(LPK_ROOTS2)/keys/
	@{ echo '# SCRATCH ONLY -- generated by the test-pkg-roots rule from the signer'; \
	   echo '# field of build/foreign.lpk. Never in tools/pkgroots/, never on an ISO.'; \
	   od -An -tx1 -j128 -N32 -v $(BUILD)/foreign.lpk | tr -d ' \n'; echo; \
	 } > $(LPK_ROOTS2)/keys/zz-scratch-foreign.pub
	python3 tools/genpkgroots.py $(LPK_ROOTS2)/keys $@

$(LPK_ROOTS2)/pkgsig.c: c/crypto/trust/pkgsig.c
	@mkdir -p $(LPK_ROOTS2)
	cp $< $@

test-pkg-roots: $(BUILD) $(LPK_FIXTURES) $(LPK_ROOTS2)/pkgroots.inc $(LPK_ROOTS2)/pkgsig.c
	@echo "--- control: the SHIPPED one-root trust set ---"
	$(CC) $(LPK_TEST_CF) -o $(BUILD)/pkg_roots1 $(LPK_TEST_SRC) \
	  c/crypto/trust/pkgsig.c $(LPK_TEST_INC)
	$(BUILD)/pkg_roots1 $(BUILD)/hello.lpk $(BUILD)/tampered.lpk $(BUILD)/foreign.lpk refused
	@echo
	@echo "--- the same fixtures against a SCRATCH two-root trust set ---"
	$(CC) $(LPK_TEST_CF) -o $(BUILD)/pkg_roots2 $(LPK_TEST_SRC) \
	  $(LPK_ROOTS2)/pkgsig.c $(LPK_TEST_INC)
	$(BUILD)/pkg_roots2 $(BUILD)/hello.lpk $(BUILD)/tampered.lpk $(BUILD)/foreign.lpk accepted
	@echo
...
# exact text of any line it is guarding. If this ever compiles, lpk_verify is
# free to grow a second reader of the trust set again and nothing would notice.
#
# "The compile failed" is not the claim; the claim is that it failed FOR THE
# POISON, because an ordinary syntax error anywhere in pkgsig.c would otherwise
# make this control report success while the file it guards is broken. What
# rules that out is STEP 1 -- the unmodified file is compiled first and must
# succeed -- and not the grep: clang error-recovers, so a copy with both a
# syntax error and the appended read reports the poison as well, and the grep
# alone would call that a pass. Measured, not assumed. The grep and the third
# verdict stay as a cheap check that the diagnostic is the one expected, on a
# compiler that does not recover the same way.
test-pkg-poison: $(BUILD)
	@$(CC) -O1 -w -c -o $(BUILD)/pkgsig_clean.o c/crypto/trust/pkgsig.c $(LPK_TEST_INC) \
	   || { echo "test-pkg-poison: pkgsig.c does not compile AS IT IS -- nothing"; \
	        echo "                 below this line would mean anything"; exit 1; }
	@cp c/crypto/trust/pkgsig.c $(BUILD)/pkgsig_relapse.c
	@printf '\nconst uint8_t *pkgsig_relapse(void);\nconst uint8_t *pkgsig_relapse(void) { return PKG_ROOTS[0].key; }\n' >> $(BUILD)/pkgsig_relapse.c
	@if $(CC) -O1 -w -c -o $(BUILD)/pkgsig_relapse.o $(BUILD)/pkgsig_relapse.c \
	      $(LPK_TEST_INC) > $(BUILD)/pkgsig_relapse.log 2>&1; then \
	    echo "CONTROL FAILED: pkgsig.c can still read PKG_ROOTS directly below the"; \
	    echo "                accessors -- the poison is not doing anything, and the"; \
	    echo "                header's 'local to pkg_root_count/pkg_root_key' is a wish"; \
	    exit 1; \
	 elif grep -qi 'poisoned identifier' $(BUILD)/pkgsig_relapse.log; then \
	    echo "control ok: a direct read of the built-in array is a compile error:"; \
	    grep -i 'poisoned identifier' $(BUILD)/pkgsig_relapse.log | head -3; \
	 else \
	    echo "CONTROL INCONCLUSIVE: the relapse did not compile, but NOT because of"; \
	    echo "                      the poison. Something else in pkgsig.c is broken:"; \
	    head -5 $(BUILD)/pkgsig_relapse.log; \
	    exit 1; \
	 fi
...
#
# The human-readable count below greps for EVERY ROOT_* the generator can emit.
# It used to be 'ROOT_RSA\|ROOT_EC', which does not match ROOT_ED25519 (the
# fifth character diverges: ROOT_EC vs ROOT_ED) -- and genroots.py accepts
# Ed25519 roots now. That number was right only because tools/roots/ happens to
# hold 105 RSA + 25 EC + 0 Ed25519 today; the first Ed25519 PEM dropped in there
# would have been compiled in, trusted, and undercounted on this line. The cmp
# below gates correctly either way -- this is the number a human reads, which is
# exactly the kind that rots without failing.
check-roots: $(BUILD)
	@python3 tools/genroots.py tools/roots $(BUILD)/roots_bundle.check 2>$(BUILD)/roots.log; \
	 grep -c 'ROOT_RSA\|ROOT_EC\|ROOT_ED25519' $(BUILD)/roots_bundle.check | xargs -I{} echo "roots: {} compiled in"; \
