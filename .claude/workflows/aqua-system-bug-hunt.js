export const meta = {
  name: 'aqua-system-bug-hunt',
  description: 'System-wide bug hunt across Aqua OS: per-subsystem finders + adversarial verification (read-only)',
  phases: [
...
- AquaFS cross-boot write durability degrades across repeated non-snapshot boots (use -snapshot). Known.
...
const CTX = `Aqua OS -- a from-scratch x86_64 kernel + userland (C + nasm), QEMU/TCG. You are hunting for
...
const VERDICT_SCHEMA = {
  ...
  properties: {
    ...
    regression_gate: { type: 'string', description: 'which make target(s) would catch a regression: test / test-aqs / test-shell / test-aqs-os / manual' },
