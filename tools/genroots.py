...
# Usage: genroots.py [--allow-empty] [rootsdir] [outfile]
# Both positional arguments default to the real trust store; they exist so a
# test can build a bundle containing exactly one throwaway CA
# (tests/unit/run-tls-interop.sh does this to verify chains against a locally
# generated anchor).
#
# --allow-empty permits a bundle with ZERO roots. Without it an empty result is
# refused outright (see the refusal in main() for what that costs at runtime).
# It is a flag rather than the default because the two ways of arriving at zero
# roots are not alike: run-tls-interop.sh's fail-closed block MEANS it -- it
# builds a client with no anchors to prove the client then trusts nothing --
# whereas a mistyped or stale rootsdir arrives at the same output by accident
# and wants to be stopped. An opt-in separates them by intent, which no
# inspection of the directory can do: a deliberately empty directory and a
# wrong path both simply match no *.pem.
ARGV = [a for a in sys.argv[1:] if a != "--allow-empty"]
ALLOW_EMPTY = "--allow-empty" in sys.argv[1:]
ROOTS = ARGV[0] if len(ARGV) > 0 else os.path.join(os.path.dirname(__file__), "roots")
...
def main():
    ...
    # REFUSE TO WRITE AN EMPTY TRUST STORE. This is not defensive tidying: it
    # is the one failure this generator can produce that nothing downstream
    # reports. ROOTS comes from argv[1], so any mistyped or stale path -- and,
    # measured on 2026-08-20, `genroots.py --help`, which this script does not
    # implement and therefore treats as a directory name -- globs zero PEMs,
    # emits `logit_roots[] = {}` and exits 0. The kernel then COMPILES, because
    # an empty array is legal C, and x509.c's two root loops (:422, :447) both
    # iterate `i < logit_nroots` and so fail closed: every certificate on the
    # public web becomes untrusted and every https fetch fails with a chain
    # error that names no cause. That is how this tree spent today -- the file
    # was found at 25 lines, down from 4,519, with 130 intact PEMs still in
    # tools/roots/.
    #
    # Rejected alternative: warn and write anyway, matching how a SKIPPED root
    # is handled ten lines below. A skip is a partial store, which is a real
    # state somebody may want; zero roots is never a state anybody wants, and
    # the warning would land in the same stderr nobody read the first time.
    # Exiting non-zero without touching OUT also leaves the PREVIOUS good
    # bundle in place, so a bad invocation costs nothing at all.
    if not rows and not ALLOW_EMPTY:
        print(f"genroots: REFUSING to write an empty trust store to {OUT}\n"
              f"  {ROOTS} matched no *.pem this script could parse.\n"
              f"  A zero-root bundle compiles and fails every chain verification\n"
              f"  at runtime, naming no cause. Check the roots directory argument.\n"
              f"  If you MEANT zero roots (proving a client fails closed), pass\n"
              f"  --allow-empty.",
              file=sys.stderr)
        return 1
...
if __name__ == "__main__":
    # sys.exit(main()) rather than a bare main(): the empty-store refusal above
    # returns 1, and a discarded return value would make that refusal print a
    # message and still exit 0 -- i.e. the Makefile rule that regenerates the
    # bundle would carry on as if it had succeeded, which is the exact failure
    # the refusal exists to stop. main() falls off its end (None -> exit 0) on
    # the success path.
    sys.exit(main())
